...
class Shape
  ...
  def needs_fall_update?
    ...
    if ( @game.button_down?(Gosu::KbDown) )
	  ...
	  updateInterval = 200 - @game.level*90
	end
  ...
  #sedangkan jika pengguna tidak menekan tombol maka interval jatuh akan 200- game.level*90
...
class LevelHard < Gosu::Window
  ...
  def update
    if ( @state == STATE_PLAY )
	  ...
	  @level = 1
	  self.caption = "My Tetris My Adventure : Hard"
	else 
  ...
  def draw
	...
	if @state == STATE_GAMEOVER
	   ...
	   text.draw(146,380,3)
	   text1 = Gosu::Image.from_text(self, "Space to Start Again", "Arial", 30)
	   text1.draw(90,415,3)
